...
(function() {
  var async, createNames, createPrdeicatorNames, createRels, es, getChildrenOrgs, getChildrenPeople, getName, getNames, init_people_query, mongo, _ES_URI, _SPARQL_URI, _iter, _iterSubjects, _names, _q, _q_children_orgs, _q_children_people, _q_org_name, _q_person_name, _q_predicator_name;
  ...
  createPrdeicatorNames = function(domain, items) {
    return async.map(items, (function(i, ck) {
      return _q_predicator_name(_SPARQL_URI, i, ck);
      ...
      if (!err) {
        ...
        for (_i = 0, _len = results.length; _i < _len; _i++) {
          ...
          _results.push(es.bulk(_ES_URI, "predicators", data, function() {}));
        }
        return _results;
      }
    });
  ...
  createRels = function(domain, predicatorType, rels) {
    ...
    for (_i = 0, _len = rels.length; _i < _len; _i++) {
      ...
      if (r) {
        ...
        if (items.length > 0) {
          ...
          createPrdeicatorNames(domain, items.map(function(m) {
            return m.predicator;
          }));
