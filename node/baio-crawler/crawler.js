...
(function() {
  var amqp, async, confirm, doneLog, isSkipInitial, log, parseData, parseLevel, push2Amqp, query, req, request, requestAndParse, skipInitialState, start, webQuery, _opts, _parse;
    ...
    return function(err) {
      ...
      if (!err) {
        ...
        if (!_opts.failed) {
          ...
          attempt = data._failed_attempt ? data._failed_attempt + 1 : 1;
            ...
            onPop: function(data, ack) {
              if (_opts.slaveLevel === -1 || data.level === _opts.slaveLevel) {
                ...
                if (data.level !== void 0) {
                  return parseLevel(data.level, data.url, confirm(data, ack));
                } else {
                  return parseData(data, confirm(data, ack));
                }
              } else {
                return ack(true);
              }
