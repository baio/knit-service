      ...
      parser.parseKeys data, (err, keys) ->
        ...
        if !err
          if keys.people.length
            q.push getPediaQueryData queries.personNameReq.replace("{0}", (keys.people.map((m) -> "?s=<#{m}>")).join(" or ")), 0, type : "person"
          if keys.orgs.length
            q.push getPediaQueryData queries.orgNameReq.replace("{0}", (keys.orgs.map((m) -> "?s=<#{m}>")).join(" or ")), 0, type : "org"
        done err, q
    else if level == 0
